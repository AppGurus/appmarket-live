          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              echo '<div id="option-tree-settings-api">';
				...
				echo '<span class="option-tree-ui-button left vesion ">Vesion: ' . $current_theme->get('Version') . '</span>';
				...
				if(leafcolor_check_for_update(PARENT_THEME) == 1){
					echo '<button class="button right update" style="margin:6px" name="leafcolor_update_theme" type="submit" value="leafcolor_update_theme" onclick="jQuery(\'#option-tree-settings-api\').attr(\'action\',\'themes.php?page=ot-theme-options&leafcolor_update_theme=true\')">' . __('New Version Found! Update Theme?','leafcolor') . '</button>';
				}
